...
# 🔥 фикс для reply (НЕ ЛОМАЕМ FORWARD)
message_user_map = {}
...
def all_messages(message):
    ...
    # 🔴 АВТО-ОТВЕТ ЕСЛИ НЕТ USERNAME
    if uid not in MODS:
        if not username:
            bot.send_message(
                uid,
                "⚠️ Укажите свой @username в Telegram!\n\n"
                "Без него ваше объявление не будет принято модераторами."
            )
            return

    # сохраняем для reply FIX
