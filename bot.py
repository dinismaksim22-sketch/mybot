...
def callback_handler(call):  
    try:  
        ...
        if mod_id not in MODS and mod_id != SUPERADMIN:  
            ...
            if not post:  
                bot.answer_callback_query(call.id, "❌ Ошибка: Пост не найден!", show_alert=True)  
                return  
  
            bot.answer_callback_query(call.id, "✅ Публикую без подписей...")  
  
            try:  
                # --- СЕКРЕТ СОХРАНЕНИЯ ПРЕМИУМ ЭМОДЗИ ---
                if post["type"] in ["text", "photo", "video"]:  
                    # Мы копируем сообщение напрямую из чата пользователя.
                    # Метод copy_message НЕ добавляет надпись "Переслано от".
                    # Важно: НЕ передаем parse_mode, чтобы не сломать Premium-сущности.
                    bot.copy_message(
                        chat_id=CHANNEL_ID, 
                    ...
                    # Для альбомов мы используем уже сформированные InputMedia
                    # в которых прописаны caption_entities (они хранят премиум-эмодзи)
